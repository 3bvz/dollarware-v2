...
do 
    ...
    do
        ...
        do
            ...
            icon.ImageColor3 = theme.Secondary
            icon.Name = '#icon'
            icon.Position = UDim2.new(1, -2, 0, 0)
            icon.Rotation = 180
            ...
            optionTemplate.BackgroundColor3 = theme.Button1
            optionTemplate.BackgroundTransparency = 1
            optionTemplate.Font = 'Ubuntu'
            optionTemplate.Name = '#option'
            optionTemplate.Size = UDim2.new(1, -4, 0, 16)
            ...
            optionTemplate.TextWrapped = false
            optionTemplate.TextXAlignment = 'Left'
            optionTemplate.TextYAlignment = 'Center'
            optionTemplate.Visible = false
            optionTemplate.ZIndex = 40
            optionTemplate.Parent = container

            local round = Instance.new('UICorner')
            round.CornerRadius = UDim.new(0, rounding and 2 or 0)
            round.Name = '#round'
            round.Parent = optionTemplate
            ...
            instances.optionTemplate = optionTemplate
        ...
        dropdown.open = function(self)
            ...
            local menuHeight = math.min(optionCount, self.maxVisible) * 18 + 6
            local frameHeight = 20 + menuHeight

            tween(self.instances.menu, {Size = UDim2.new(1, -6, 0, menuHeight)}, 0.3, 1)
            tween(self.instances.controlFrame, {Size = UDim2.new(1, 0, 0, frameHeight)}, 0.3, 1)
        end
        ...
        dropdown.setSelected = function(self, value)
            self.selected = value
            self.instances.label.Text = tostring(value)
            self:fireEvent('onSelect', value)
            return self
        end

        dropdown.setOptions = function(self, newOptions)
            for _, optData in ipairs(self.optionInstances) do
                optData.inst:Destroy()
            end
            table.clear(self.optionInstances)
            self.options = {}
        ...
        dropdown.addOption = function(self, optionText)
            optionText = tostring(optionText)
            table.insert(self.options, optionText)

            local template = self.instances.optionTemplate
            local newOpt = template:Clone()
            newOpt.Name = '#opt-' .. #self.options
            newOpt.Text = optionText
            newOpt.Visible = true
            newOpt.LayoutOrder = #self.options
            newOpt.Parent = template.Parent

            newOpt.MouseEnter:Connect(function()
                tween(newOpt, {BackgroundTransparency = 0, BackgroundColor3 = theme.Button2}, 0.15, 1)
            end)
            newOpt.MouseLeave:Connect(function()
                tween(newOpt, {BackgroundTransparency = 1}, 0.15, 1)
            end)
            newOpt.MouseButton1Click:Connect(function()
                self.selected = optionText
                self.instances.label.Text = optionText

                newOpt.TextColor3 = theme.Primary
                tween(newOpt, {TextColor3 = theme.TextPrimary}, 0.5, 1)

                self:close()
                self:fireEvent('onSelect', optionText)
            end)

            table.insert(self.optionInstances, {
                text = optionText,
                inst = newOpt
            })
        ...
        dropdown.removeOption = function(self, optionText)
            optionText = tostring(optionText)
            for i, optData in ipairs(self.optionInstances) do
                if (optData.text == optionText) then
                    optData.inst:Destroy()
                    table.remove(self.optionInstances, i)
                    table.remove(self.options, i)
                    break
                end
            end

            if (self.selected == optionText) then
                self.selected = nil
                self.instances.label.Text = 'select...'
            end
            return self
        end
        ...
        dropdown.new = function(self)
            ...
            instances.optionTemplate = instances.menu['#container']['#option']
        ...
        elemClasses.section.addDropdown = function(self, settings, callback)
            ...
            local s_default = settings.default or nil
            local s_maxVisible = settings.maxVisible or 5

            local new = dropdown:new()
            new.section = self
            new.name = s_title
            new.maxVisible = s_maxVisible
            table.insert(self.controls, new)

            new.instances.label.Text = s_default or s_title
            new.instances.controlFrame.Parent = self.instances.controlMenu
